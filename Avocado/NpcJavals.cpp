...
//-----------------------------------------------------------------------------
void NpcJavals::Render(class AdLevel* pLvl) {
	...
	if(m_bTriggered) {
		...
		SDL_Point pnt = {m_recTrigger.x-8*25-4, AdBase::GetHeight()-8*9+4};
		AdScreen::DrawSprite(pnt, bkg);
		SDL_FreeSurface(bkg);


		//>^.^< +^.^+ =^.^= ~^.^~
		SDL_Surface* msg = AdSpriteManager::BuildSprite("JAVALS: THIS ANCIENT CITTIE WAS ONCE");
		SDL_Surface* msg0 = AdSpriteManager::BuildSprite("THE SUCKLE KING. HIS SWEET LOVING");
		SDL_Surface* msg1 = AdSpriteManager::BuildSprite("ATTITUDE ALLOWED FOR THE END OF ALL");
		SDL_Surface* msg2 = AdSpriteManager::BuildSprite("SADNESS ACROSS THE LAND.");
